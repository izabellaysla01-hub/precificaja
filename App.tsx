...
export default function App() {
  ...
  const enviarZap = (p: any) => {
    ...
    const msg = `*ORÇAMENTO - PrecificaJá*%0A---%0A*Cliente:* ${cli?.nome || 'Cliente'}%0A*Produto:* ${p.nomeProd}%0A*Qtd:* ${p.qtdPed || 1} un%0A*Prazo:* ${dataPrazo}%0A*Valor:* R$ ${p.preco}%0A---%0AObrigado!`;
          ...
          <div className="bg-white p-6 rounded-[35px] shadow-xl border mt-2">
            ...
            <div className="mb-6">
               ...
               <select className="w-full p-4 bg-slate-50 rounded-2xl outline-none mb-3" onChange={e => {
                  ...
                  <option value="">+ Materiais (folhas/unidades)</option>
                  ...
                  {matsNoPed.map((m, i) => (
                    <div key={i} className="flex justify-between items-center bg-purple-50 p-3 rounded-2xl border border-purple-100 text-purple-700 font-bold text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        <input type="number" className="w-10 bg-white rounded-lg p-1 text-center" value={m.qtdUsada} onChange={e => {
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div><label className="text-[10px] font-bold text-orange-500 uppercase flex items-center gap-1"><Clock size={10}/> Tempo (minutos)</label>
              <input type="number" className="w-full p-3 bg-slate-50 rounded-xl outline-none" value={tGasto} onChange={e => setTGasto(e.target.value)} /></div>
              <div><label className="text-[10px] font-bold text-orange-500 uppercase flex items-center gap-1"><DollarSign size={10}/> Valor da Hora</label>
              <input type="number" className="w-full p-3 bg-slate-50 rounded-xl outline-none" value={vHora} onChange={e => setVHora(e.target.value)} /></div>
            </div>
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div><label className="text-[10px] font-bold text-orange-500 uppercase flex items-center gap-1"><Percent size={10}/> Lucro %</label>
              <input type="number" className="w-full p-3 bg-slate-50 rounded-xl outline-none" value={lucro} onChange={e => setLucro(e.target.value)} /></div>
              <div><label className="text-[10px] font-bold text-orange-500 uppercase flex items-center gap-1"><Calendar size={10}/> Prazo</label>
              <input type="date" className="w-full p-3 bg-slate-50 rounded-xl text-xs outline-none" value={prazo} onChange={e => setPrazo(e.target.value)} /></div>
            </div>
            <div className="mb-10">
               <label className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1"><Tag size={10}/> Valor do Desconto (R$)</label>
               <input className="w-full p-4 bg-slate-50 rounded-2xl outline-none" type="number" value={desconto} onChange={e => setDesconto(e.target.value)} />
            </div>
            <div className="flex items-center justify-between border-t pt-6">
              ...
              <div className="flex gap-2">
                <button onClick={async () => {
                   ...
                   await addDoc(collection(db, "pedidos"), { nomeProd, preco: precoFinal, clienteId: clienteSel, pagamento, prazo, qtdPed, userId: user.uid, data: new Date().toLocaleDateString() });
                ...
                <button onClick={() => enviarZap({nomeProd, preco: precoFinal, clienteId: clienteSel, prazo, qtdPed})} className="bg-emerald-500 text-white p-4 rounded-3xl"><MessageCircle/></button>
              </div>
            </div>
          </div>
        )}
               ...
               return (
                 <div key={p.id} className="bg-white p-5 rounded-[30px] shadow-sm flex justify-between items-center border border-slate-50">
                   <div>
                      ...
                      <p className="font-bold text-xs">{p.nomeProd}</p>
                      <p className="text-[8px] text-slate-300 font-bold uppercase">{p.data}</p>
                   </div>
                   <div className="flex items-center gap-2">
          ...
          <div className="space-y-4 pt-2">
            <div className="bg-white p-8 rounded-[40px] shadow-md border border-slate-50">
              ...
              <div className="flex gap-3 mb-6">
                ...
                <input type="number" placeholder="Folhas" className="w-24 p-4 bg-slate-50 rounded-2xl outline-none text-center" value={novoMat.qtd} onChange={e => setNovoMat({...novoMat, qtd: e.target.value})} />
              </div>
              <button onClick={async () => {
                ...
                alert("Salvo!");
              }} className="w-full bg-orange-500 text-white p-5 rounded-2xl font-black uppercase text-xs">Salvar no Estoque</button>
            </div>
            {materiais.map(m => (
           ...
           <div className="space-y-4 pt-2">
            <div className="bg-white p-8 rounded-[40px] shadow-md border border-slate-50">
              ...
              <button onClick={async () => {
                ...
                setNovoCli({ nome: '', zap: '' }); alert("Salvo!");
              }} className="w-full bg-orange-500 text-white p-5 rounded-2xl font-black uppercase text-xs">Salvar Cliente</button>
            </div>
            {clientes.map(c => <div key={c.id} className="bg-white p-5 rounded-3xl flex justify-between items-center border border-slate-50"><span className="font-bold">{c.nome}</span><button onClick={() => deleteDoc(doc(db, "clientes", c.id))} className="text-red-200 p-2"><Trash2 size={18}/></button></div>)}
          </div>
        )}
      </main>
