...
export default function App() {
  ...
  const gerarPDF = (p: any) => {
    ...
    elemento.innerHTML = `
      <div style="padding: 35px; font-family: sans-serif; color: #334155; max-width: 750px; margin: 0 auto;">
        ...
        <p style="margin: 0;"><strong>Cliente:</strong> ${cli?.nome || 'Cliente não informado'}</p>
            ...
            {produtosPublicos.map(p => {
              ...
              return (
                <div key={p.id} className="bg-white p-4 rounded-[35px] border shadow-sm flex gap-4 items-center">
                  <div className="w-24 h-24 rounded-2xl bg-slate-100 overflow-hidden flex items-center justify-center text-slate-300 shrink-0">{p.urlImagem ? <img src={p.urlImagem} alt={p.nome} className="w-full h-full object-cover" /> : <ImageIcon size={30} />}</div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32 font-sans text-slate-700 w-full relative overflow-x-hidden">
      ...
      {/* MENU HAMBÚRGUER REINTEGRADO COM TODAS AS ABAS DO APP */}
      ...
      <div className="p-4 max-w-xl mx-auto w-full">
        ...
        {/* ABA DE ORÇAMENTOS COM SELETOR DUPLO */}
        {activeTab === 'criar' && (
