...
export default function App() {
  ...
  // Estados de criação de um Molde Novo
  ...
  // REGRAS DE BAIXA AUTOMÁTICA ATUALIZADAS E CORRIGIDAS
  const confirmarVendaPedido = async (pedido: any) => {
    ...
    alert("Venda confirmada e materiais deduzidos! 📦📈");
  };
  ...
  const carregarPedidoParaEdicao = (p: any) => {
    ...
    if (p.materialsUsados || p.materiaisUsados) {
      const lista = p.materialsUsados || p.materiaisUsados;
      setMatsNoPed(lista.map((mSalvo: any) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32 font-sans text-slate-700 w-full relative overflow-x-hidden">
      ...
      {/* MENU HAMBÚRGUER COMPLETO COM ABSOLUTAMENTE TUDO DO APP */}
