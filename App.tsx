...
export default function App() {
  ...
  const enviarZap = (p: any) => {
    ...
    // CORREÇÃO: Agora o sistema busca o preço de 3 lugares diferentes para não quebrar
    const totalNum = Number(p.preco || p.precoManual || 0);

    let htmlLinhasTabela = '';

    // Lógica para tratar itens
    if (p.itensCombo && Array.isArray(p.itensCombo) && p.itensCombo.length > 0) {
      htmlLinhasTabela = p.itensCombo.map((item) => {
        const qtd = Number(item.qtd || 1);
        const precoVenda = Number(item.precoVenda || 0);
        return `
          <tr style="border-bottom: 1px solid #f1f5f9; font-size: 14px;">
            <td style="padding: 15px 5px; font-weight: bold;">${item.nome}</td>
            <td style="padding: 15px 5px; text-align: center;">${qtd}</td>
            <td style="padding: 15px 5px; text-align: right;">R$ ${precoVenda.toFixed(2)}</td>
            <td style="padding: 15px 5px; text-align: right;">R$ ${(qtd * precoVenda).toFixed(2)}</td>
          </tr>
        `;
      }).join('');
    } else {
      // Se não for combo, usa o nomeProd
      htmlLinhasTabela = `
        <tr>
          <td style="padding: 15px 5px;">${p.nomeProd || 'Produto sem nome'}</td>
          <td style="padding: 15px 5px; text-align: center;">${p.qtdPed || 1}</td>
          <td style="padding: 15px 5px; text-align: right;">R$ ${totalNum.toFixed(2)}</td>
          <td style="padding: 15px 5px; text-align: right;">R$ ${totalNum.toFixed(2)}</td>
        </tr>
      `;
    }
    ...
    const elemento = document.createElement('div');
    elemento.innerHTML = `
      <div style="padding: 35px; font-family: sans-serif; color: #334155;">
        <h1 style="color: #7c3aed;">${cabecalhoNomeHtml}</h1>
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tbody>${htmlLinhasTabela}</tbody>
        </table>
        <h2 style="text-align: right; margin-top: 20px;">Total: R$ ${totalNum.toFixed(2)}</h2>
      </div>
    `;
    
    const opcoes = { margin: 10, filename: `Orcamento.pdf`, html2canvas: { scale: 2 }, jsPDF: { format: 'a4' } };
