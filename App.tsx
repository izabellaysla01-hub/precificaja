            ...
            <div className="bg-gradient-to-tr from-purple-800 to-purple-600 p-5 rounded-[35px] text-white shadow-md border border-purple-900 space-y-3.5 w-full">
              ...
              <div className="border-t border-purple-500/30 pt-2.5 w-full">
                ...
                <div className="flex gap-2 w-full">
                  ...
                  {/* AJUSTADO: Mudamos o bg-orange-50 para bg-purple-700 e adicionamos hover para o botão se destacar de verdade */}
                  <button onClick={async () => {
                    if(!zapDonaConta.trim()) return alert("Digite o número!");
                    try { await setDoc(doc(db, "configuracoes_loja", user.uid), { whatsapp: zapDonaConta.trim() }, { merge: true }); alert("WhatsApp salvo com sucesso!"); } 
                    catch { alert("Erro ao salvar."); }
                  }} className="bg-purple-700 hover:bg-purple-800 text-white text-xs font-black uppercase px-4 rounded-xl shadow transition-colors">Salvar</button>
                </div>
              </div>
            </div>
