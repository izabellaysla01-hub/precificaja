...
export default function App() {
  ...
  const enviarZap = (p: any) => {
    ...
    const msg = `*ORÇAMENTO - Loop Creative*%0A---%0A*Cliente:* ${cli?.nome || 'Cliente'}%0A*Produto:* ${p.nomeProd}%0A*Qtd:* ${p.qtdPed || 1} un%0A*Prazo:* ${dataPrazo}%0A*VALOR TOTAL:* R$ ${p.preco}%0A---%0AObrigado!`;
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32 font-sans text-slate-700">
      ...
      <header className="bg-white p-4 flex justify-between items-center shadow-sm border-b sticky top-0 z-50">
        <div className="font-black text-purple-700 text-lg flex items-center gap-2 tracking-tighter">
          <Calculator size={22}/> PrecificaJá
        </div>
        <button onClick={() => signOut(auth)} className="flex items-center gap-1 bg-red-50 text-red-500 px-3 py-2 rounded-xl text-xs font-bold transition-all active:scale-95">
          <LogOut size={14}/> SAIR
        </button>
      </header>
      ...
      <main className="p-4 max-w-xl mx-auto">
        {activeTab === 'criar' && (
          <div className="bg-white p-6 rounded-[35px] shadow-xl border mt-2 animate-in fade-in duration-500">
            ...
            <div className="mb-6">
               ...
               <select className="p-4 bg-slate-50 rounded-2xl outline-none w-full mb-4 font-medium" onChange={e => setClienteSel(e.target.value)} value={clienteSel}>
               ...
               <select className="w-full p-4 bg-slate-50 rounded-2xl outline-none mb-3 border-2 border-purple-50 focus:border-purple-200" onChange={e => {
                  ...
                  {matsNoPed.map((m, i) => (
                    <div key={i} className="flex justify-between items-center bg-purple-50 p-3 rounded-2xl border border-purple-100 text-purple-700 font-bold text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        <input type="number" className="w-12 bg-white rounded-lg p-1 text-center outline-none" value={m.qtdUsada} onChange={e => {
                           const nova = [...matsNoPed]; nova[i].qtdUsada = e.target.value; setMatsNoPed(nova);
                        }} />
                        <button onClick={() => setMatsNoPed(matsNoPed.filter((_, idx) => idx !== i))}><X size={16} className="text-purple-300"/></button>
                      </div>
                    </div>
                  ))}
               </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div><label className="text-[10px] font-bold text-orange-500 uppercase flex items-center gap-1"><Clock size={10}/> Tempo Gasto (min)</label>
              ...
              <div><label className="text-[10px] font-bold text-orange-500 uppercase flex items-center gap-1"><DollarSign size={10}/> Valor da Hora (R$)</label>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div><label className="text-[10px] font-bold text-orange-500 uppercase flex items-center gap-1"><Percent size={10}/> Margem Lucro (%)</label>
              ...
              <div><label className="text-[10px] font-bold text-orange-500 uppercase flex items-center gap-1"><Calendar size={10}/> Prazo de Entrega</label>
            ...
            <div className="mb-10">
               <label className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1 ml-1"><Tag size={10}/> Valor do Desconto (R$)</label>
            ...
            <div className="flex items-center justify-between border-t pt-8">
              ...
              <div className="flex gap-2">
                <button onClick={async () => {
                   ...
                   alert("Pedido Salvo!"); setActiveTab('pedidos');
                }} className="bg-orange-500 text-white px-8 py-5 rounded-[25px] font-black uppercase text-xs shadow-lg shadow-orange-100 active:scale-95 transition-all">Salvar</button>
                <button onClick={() => enviarZap({nomeProd, preco: precoFinal, clienteId: clienteSel, prazo, qtdPed})} className="bg-emerald-500 text-white p-5 rounded-[25px] shadow-lg shadow-emerald-100 active:scale-95 transition-all"><MessageCircle/></button>
              </div>
            </div>
          </div>
        )}

        {/* HISTÓRICO COM BOTÃO WHATSAPP E EXCLUIR */}
        {activeTab === 'pedidos' && (
          <div className="space-y-3 pt-2 animate-in slide-in-from-right duration-300">
            <h2 className="text-purple-700 font-bold mb-4 flex items-center gap-2"><History size={20}/> Histórico de Vendas</h2>
            {pedidos.length === 0 ? <p className="text-center py-20 text-slate-300 font-bold uppercase text-xs tracking-widest">Nenhum pedido salvo</p> : pedidos.map(p => {
               ...
               return (
                 <div key={p.id} className="bg-white p-5 rounded-[30px] shadow-sm flex justify-between items-center border border-slate-50">
                   <div className="max-w-[150px]">
                      <p className="font-black text-[10px] uppercase text-purple-700 leading-none mb-1 truncate">{cli?.nome || 'Sem Nome'}</p>
                      <p className="font-bold text-slate-700 text-sm truncate">{p.nomeProd}</p>
                   ...
                   <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => enviarZap(p)} className="text-emerald-500 p-2 bg-emerald-50 rounded-xl active:scale-90"><MessageCircle size={20}/></button>
                      <button onClick={() => confirmarExcluir('pedido', p.id)} className="text-red-300 p-2 hover:text-red-500 active:scale-90"><Trash2 size={20}/></button>
                   </div>
                 </div>
               );
            })}
        ...
        {/* ESTOQUE COM UNITÁRIO E EDITAR */}
        {activeTab === 'materiais' && (
          <div className="space-y-4 pt-2 animate-in slide-in-from-left duration-300">
            <div className="bg-white p-8 rounded-[40px] shadow-md border border-slate-50">
              ...
              }} className="w-full bg-orange-500 text-white p-5 rounded-2xl font-black shadow-lg uppercase text-xs">Salvar Material</button>
            </div>
            {materiais.map(m => (
              <div key={m.id} className="bg-white p-5 rounded-3xl flex justify-between items-center border border-slate-50">
                <div>
                  <p className="font-bold text-slate-700">{m.nome}</p>
                  <p className="text-orange-500 font-black text-sm">R$ {(Number(m.valor)/Number(m.qtd)).toFixed(2)} <span className="text-[9px] font-normal text-slate-300">/ un.</span></p>
                </div>
                <div className="flex gap-1">
                  <button onClick={() => setNovoMat({id: m.id, nome: m.nome, valor: m.valor, qtd: m.qtd})} className="text-orange-400 p-2 active:scale-90"><Edit2 size={20}/></button>
                  <button onClick={() => confirmarExcluir('material', m.id)} className="text-red-200 p-2 active:scale-90"><Trash2 size={20}/></button>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* CLIENTES COM EXCLUIR */}
        {activeTab === 'clientes' && (
           <div className="space-y-4 pt-2 animate-in slide-in-from-bottom duration-300">
            <div className="bg-white p-8 rounded-[40px] shadow-md border">
              ...
              <input placeholder="WhatsApp (DDD + Número)" className="w-full p-4 bg-slate-50 rounded-2xl mb-6 outline-none" value={novoCli.zap} onChange={e => setNovoCli({...novoCli, zap: e.target.value})} />
              <button onClick={async () => {
            ...
            {clientes.map(c => <div key={c.id} className="bg-white p-5 rounded-3xl flex justify-between items-center border border-slate-50 font-bold"><span className="ml-2">{c.nome}</span><button onClick={() => confirmarExcluir('cliente', c.id)} className="text-red-200 p-2"><Trash2 size={20}/></button></div>)}
          </div>
        )}
      </main>
      ...
      <div className="fixed bottom-6 w-full flex justify-around px-4 items-center">
          <button onClick={() => setActiveTab('materiais')} className={`p-4 rounded-2xl transition-all ${activeTab === 'materiais' ? 'bg-orange-500 text-white shadow-lg shadow-orange-100 scale-110' : 'bg-white text-slate-300'}`}><Package size={22}/></button>
          <button onClick={() => setActiveTab('clientes')} className={`p-4 rounded-2xl transition-all ${activeTab === 'clientes' ? 'bg-orange-500 text-white shadow-lg shadow-orange-100 scale-110' : 'bg-white text-slate-300'}`}><User size={22}/></button>
          ...
          <button onClick={() => setActiveTab('pedidos')} className={`p-4 rounded-2xl transition-all ${activeTab === 'pedidos' ? 'bg-orange-500 text-white shadow-lg shadow-orange-100 scale-110' : 'bg-white text-slate-300'}`}><History size={22}/></button>
      </div>
    </div>
  );
}
