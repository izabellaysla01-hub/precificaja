...
export default function App() {
  ...
  const [activeTab, setActiveTabState] = useState<'inicio' | 'materiais' | 'criar' | 'pedidos' | 'clientes' | 'catalogo' | 'moldes'>('inicio');
  ...
  // Estados da Calculadora Livre Normal
  ...
  // Estados do Orçamento via Moldes Salvos
  ...
  // CÁLCULO CUSTO E LUCRO POR UNIDADE DO MOLDE
  ...
  // CÁLCULO GERAL DO LOTE NO MOMENTO DO ORÇAMENTO POR MOLDES
  ...
  const gerarPDF = (p: any) => {
    ...
    elemento.innerHTML = `
      <div style="padding: 35px; font-family: sans-serif; color: #334155; max-width: 750px; margin: 0 auto;">
        ...
        <div style="background-color: #f8fafc; padding: 15px; border-radius: 16px; margin-bottom: 25px; border: 1px solid #f1f5f9; font-size: 13px;">
          <p><strong>Emissão:</strong> ${dataEmissao} | <strong>Prazo:</strong> ${dataPrazo}</p>
        </div>
        <p style="font-size: 16px; font-weight: bold;">Descrição: ${p.nomeProd}</p>
        <h2 style="color: #7c3aed; font-size: 24px; font-weight: 900; margin-top: 20px;">VALOR TOTAL: R$ ${totalNum.toFixed(2)}</h2>
      </div>
    `;
  ...
  // REGISTRO DE VENDA COM BAIXA INTELIGENTE NO ESTOQUE (CALCULADORA E MOLDES)
  const confirmarVendaPedidoEBaixarEstoque = async (pedido: any) => {
    if (pedido.moldesVinculados && pedido.moldesVinculados.length > 0) {
      for (const itemMolde of pedido.moldesVinculados) {
        if (itemMolde.materiaisUsados && itemMolde.materiaisUsados.length > 0) {
          for (const matGasto of itemMolde.materiaisUsados) {
            ...
            if (matDoBanco) {
              const estoque />= Number(matDoBanco.qtdAtual || 0);
              const totalDescontar = Number(matGasto.qtdUsada || 0) * Number(itemMolde.qtd || 1);
              await updateDoc(doc(db, "materiais", matGasto.id), { qtdAtual: Math.max(0, estoque - totalDescontar) });
            }
          }
        }
      }
      ...
      for (const m of pedido.materiaisUsados) {
        ...
        if (matDoBanco) {
          ...
          await updateDoc(doc(db, "materials" || "materiais", m.id), { qtdAtual: Math.max(0, estoqueFiscal - gastoTotal) });
        }
      }
    }
    await updateDoc(doc(db, "pedidos", pedido.id), { status: 'Vendido 💰' });
    alert("Venda registrada e materiais deduzidos! 📦📈");
  };
  ...
  const handleUploadImagem = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch { alert("Erro ao subir a foto!"); } finally { setSubindoImagem(false); }
  };
  ...
  // FORMULÁRIO DA CALCULADORA LIVRE
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32 font-sans text-slate-700 w-full relative overflow-x-hidden">
      ...
      <header className="bg-white p-4 flex justify-between items-center shadow-sm sticky top-0 z-40 w-full">
        <button onClick={() => setIsMenuOpen(true)} className="p-2 text-slate-700 hover:text-purple-700"><Menu size={24} /></button>
               ...
               return (
                 <div key={p.id} className="bg-white p-5 rounded-[30px] shadow-sm flex flex-col gap-3 border w-full">
                   ...
                   <div className="flex items-center justify-end border-t pt-2 gap-1 w-full">
                      {ehPendente && (
                        <button onClick={() => confirmarVendaPedidoEBaixarEstoque(p)} className="text-emerald-600 p-2 bg-emerald-50 rounded-xl text-xs font-bold flex items-center gap-1 mr-auto active:scale-95"><CheckCircle size={16}/> Confirmar Venda</button>
                      )}
          ...
          <div className="space-y-4 pt-2 w-full">
            <div className="bg-gradient-to-tr from-purple-800 to-purple-600 p-6 rounded-[35px] text-white shadow-lg border border-purple-900 space-y-4 w-full">
              <div className="w-full">
                ...
                <div className="mt-2 bg-black/10 p-3 rounded-xl text-xs break-all font-mono border border-purple-500/30 w-full">{linkDoCatalogoDestaCliente}</div>
                <div onClick={copiarLinkCatalogo} className="mt-2.5 w-full bg-white text-purple-800 font-bold p-3 rounded-xl text-xs uppercase shadow flex items-center justify-center gap-2">Copiar Link do Catálogo</div>
              </div>
            </div>
            <div className="bg-white p-6 rounded-[35px] shadow-md border w-full">
              <h2 className="text-purple-700 font-bold mb-4 flex items-center gap-2 uppercase text-xs tracking-widest"><BookOpen size={18}/> Novo Item de Vitrine</h2>
              <div className="mb-4 flex flex-col items-center justify-center border-2 border-dashed rounded-3xl p-4 bg-slate-50 relative min-h-[140px] w-full">
                {novoProdCatalogo.urlImagem ? (
                  ...
                  <label className="cursor-pointer flex flex-col items-center gap-2 text-slate-400 hover:text-purple-600 w-full h-full flex justify-center"><Camera size={22} /><span className="text-xs font-bold uppercase tracking-wide text-[10px]">{subindoImagem ? 'Subindo...' : '📸 Adicionar Foto'}</span><input type="file" accept="image/*" className="hidden" onChange={handleUploadImagem} disabled={subindoImagem} /></label>
                )}
              </div>
              ...
              <input type="number" placeholder="Preço" className="w-full p-4 bg-slate-50 rounded-2xl mb-4 outline-none font-bold text-purple-700" value={novoProdCatalogo.precoVenda} onChange={e => setNovoProdCatalogo({...novoProdCatalogo, precoVenda: e.target.value})} />
              <button onClick={async () => {
              ...
              }} className="w-full bg-purple-700 text-white p-4 rounded-2xl font-black uppercase text-xs" disabled={subindoImagem}>Salvar no Catálogo 📖</button>
            </div>
          </div>
        )}
      ...
      {/* MENU INFERIOR COMPACTO DA FOTO (INÍCIO, ORÇAR, HISTÓRICO) */}
