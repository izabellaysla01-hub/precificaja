...
export default function App() {
  ...
  const [corLojaPerfil, setCorLojaPerfil] = useState('#7c3aed'); // Estado da cor customizada
      ...
      if (u) {
        getDoc(doc(db, "configuracoes_loja", u.uid)).then(docSnap => {
          if(docSnap.exists()) {
            ...
            setCorLojaPerfil(docSnap.data().corLoja || '#7c3aed'); // Carrega a cor salva do banco
          }
        });
      }
      setLoading(false);
  ...
  // MODIFICADO: Atualizado para suportar a cor dinâmica da marca de quem orçar no PDF comercial
  const gerarPDF = (p: any) => {
    ...
    // MODIFICADO: Atributo crossOrigin="anonymous" inserido para permitir renderização de mídia externa no canvas sem erros de segurança
    ...
    elemento.innerHTML = `
      <div style="padding: 35px; font-family: sans-serif; color: #334155; max-width: 750px; margin: 0 auto;">
        ...
        <div style="color: white; padding: 8px 15px; border-radius: 8px; font-size: 11px; font-weight: bold; text-transform: uppercase; margin-bottom: 12px; background-color: ${corLojaPerfil};">Dados do Cliente</div>
        <div style="background-color: #f8fafc; padding: 15px; border-radius: 16px; margin-bottom: 25px; border: 1px solid #f1f5f9;">
        ...
        <div style="color: white; padding: 8px 15px; border-radius: 8px; font-size: 11px; font-weight: bold; text-transform: uppercase; margin-bottom: 12px; background-color: ${corLojaPerfil};">Informações Básicas e Prazos</div>
        ...
        <div style="color: white; padding: 8px 15px; border-radius: 8px; font-size: 11px; font-weight: bold; text-transform: uppercase; margin-bottom: 12px; background-color: ${corLojaPerfil};">Produtos / Serviços Selecionados</div>
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
          <thead>
            <tr style="border-bottom: 2px solid #e2e8f0; text-align: left; font-size: 11px; text-transform: uppercase; color: #94a3b8;">
    ...
    `;
    // MODIFICADO: useCORS e allowTaint forçados nas opções do gerador para liberar mídias externas no canvas
  ...
  const renderCalculadoraForm = () => (
    <div className="bg-white p-6 rounded-[35px] shadow-xl border mt-2 w-full">
      ...
      <div className="flex justify-between items-center mb-6 w-full">
        {/* MODIFICADO: Estilo aplicado dinamicamente usando a cor corporativa */}
        ...
        {!pedidoEditandoId && (
          <button onClick={() => setMostrarSeletorCatalogo(!mostrarSeletorCatalogo)} className="text-xs px-3 py-1.5 rounded-xl font-black uppercase border" style={{ color: corLojaPerfil, backgroundColor: `${corLojaPerfil}10`, borderColor: `${corLojaPerfil}20` }}>
      ...
      {mostrarSeletorCatalogo && !pedidoEditandoId && (
        <div className="border p-4 rounded-3xl mb-4 text-xs space-y-2 w-full" style={{ backgroundColor: `${corLojaPerfil}05`, borderColor: `${corLojaPerfil}20` }}>
          ...
          <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto w-full">
            {produtos.map(p => (
              <div key={p.id} onClick={() => { setNomeProd(p.nome); setPrecoManual(String(p.precoVenda)); setMostrarSeletorCatalogo(false); }} className="bg-white p-2.5 rounded-xl border flex justify-between items-center cursor-pointer hover:border-slate-300 w-full">
        ...
        <>
          <div className="mb-4 w-full">
             ...
             <div className="space-y-2 w-full">
                {matsNoPed.map((m, i) => (
                  <div key={i} className="flex justify-between items-center p-3 rounded-2xl border font-bold text-xs w-full" style={{ color: corLojaPerfil, backgroundColor: `${corLojaPerfil}08`, borderColor: `${corLojaPerfil}15` }}>
                    <span>{m.nome}</span>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] opacity-75">{m.unidade || 'un'}</span>
          ...
          {equipamentos.length > 0 && (
            <div className="mb-4 w-full">
              <label className="text-[10px] font-bold uppercase ml-1 block mb-1" style={{ color: corLojaPerfil }}>🛠️ Equipamentos Ativos neste Orçamento</label>
          ...
          <div className="mb-4 w-full">
            <label className="text-[10px] font-bold uppercase ml-1 block mb-1" style={{ color: corLojaPerfil }}>📦 Custos Extras por Unidade - Opcional (R$)</label>
      ...
      <div className="mb-6 w-full">
         <label className="text-[10px] font-bold uppercase ml-1" style={{ color: corLojaPerfil }}>📝 Observações do Orçamento</label>
      ...
      {precoManual === null && (
        <div className="bg-slate-50 p-5 rounded-3xl mb-8 border border-slate-100 text-xs space-y-2.5 w-full">
          ...
          <div className="flex justify-between text-slate-800 font-bold border-t pt-2 mt-1 w-full"><span>Custo Total da Peça:</span><span>R$ {resumenFinanceiro.custoPeca}</span></div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32 font-sans text-slate-700 w-full relative overflow-x-hidden">
        ...
        <div className={`w-72 bg-white h-full shadow-2xl p-6 flex flex-col justify-between transition-transform duration-300 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`} onClick={e => e.stopPropagation()}>
          <div className="space-y-6">
            <div className="flex justify-between items-center border-b pb-4">
              {/* MODIFICADO: Título com cor da marca */}
            ...
            <nav className="flex flex-col gap-1">
              {/* MODIFICADO: Menus laterais assumem um estilo sutil baseado na cor corporativa do usuário */}
              <button onClick={() => setActiveTab('inicio')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'inicio' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><Home size={16}/> Início</button>
              <button onClick={() => setActiveTab('criar')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'criar' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><Plus size={16}/> Orçar</button>
              <button onClick={() => setActiveTab('perfil')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'perfil' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><Settings size={16}/> Perfil da Loja</button>
              <button onClick={() => setActiveTab('anotacoes')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'anotacoes' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><BookOpen size={16}/> Minhas Anotações 📝</button>
              <button onClick={() => setActiveTab('financeiro')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'financeiro' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><Calculator size={16}/> Configurações de Custos</button>
              <button onClick={() => setActiveTab('pedidos')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'pedidos' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><History size={16}/> Histórico de Orçamentos</button>
              <button onClick={() => setActiveTab('balcao')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'balcao' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><ShoppingCart size={16}/> Balcão de Vendas Rápido</button>
              <button onClick={() => setActiveTab('catalogo')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'catalogo' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><BookOpen size={16}/> Meu Catálogo Visual</button>
              <button onClick={() => setActiveTab('materiais')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'materiais' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><Package size={16}/> Armário / Insumos</button>
              <button onClick={() => setActiveTab('clientes')} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-xs transition-all" style={activeTab === 'clientes' ? { color: corLojaPerfil, backgroundColor: `${corLojaPerfil}12` } : { color: '#475569' }}><User size={16}/> Meus Clientes</button>
            </nav>
          </div>
      ...
      <header className="bg-white p-4 flex justify-between items-center shadow-sm sticky top-0 z-40 w-full">
        <button onClick={() => setIsMenuOpen(true)} className="p-2 text-slate-700 transition-colors">
          <Menu size={24} />
        </button>
      ...
      <div className="p-4 max-w-xl mx-auto w-full">
        ...
        {activeTab === 'perfil' && (
          <div className="space-y-6 pt-2 w-full">
            <div className="bg-white p-6 rounded-[35px] shadow-md border w-full">
              ...
              {/* ADICIONADO: Input picker para escolher a cor customizada */}
              ...
              <button onClick={async () => {
                try {
                  await setDoc(doc(db, "configuracoes_loja", user.uid), {
                    ...
                    corLoja: corLojaPerfil // Salva a cor no banco de dados do Firestore
                  }, { merge: true });
        ...
        {activeTab === 'financeiro' && (
          <div className="space-y-6 pt-2 w-full">
            <div className="bg-white p-6 rounded-[35px] shadow-md border w-full">
              ...
              <p className="text-slate-400 text-[11px] mb-4">Insira ou edite seus values aqui. Eles ficam salvos e você pode alterá-los quando quiser.</p>

              <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Salário Mensal Pretendido</label>
              <input type="number" className="w-full p-4 bg-slate-50 rounded-2xl mb-3 font-bold outline-none border focus:border-purple-400" style={{ color: corLojaPerfil }} value={financasFixo.salario} onChange={e => setFinancasFixo({...financasFixo, salario: e.target.value})} />
            ...
            <div className="bg-white p-6 rounded-[35px] shadow-md border w-full">
              ...
              <input placeholder="Ex: Prensa de Caneca" className="w-full p-4 bg-slate-50 rounded-2xl mb-3 outline-none" value={novoEquipamento.nome} onChange={e => setNovoEquipamento({...novoEquipamento, nome: e.target.value})} />
                  ...
                  return (
                    <div key={p.id} className="flex justify-between items-center bg-slate-900/60 p-2.5 rounded-xl border border-slate-800/80">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <button onClick={() => setCarrinhoInterno({...carrinhoInterno, [p.id]: qtdInterna + 1})} className="w-7 h-7 rounded-lg font-black text-white" style={{ backgroundColor: corLojaPerfil }}>+</button>
                      </div>
                    </div>
                  );
                })}
        ...
        {activeTab === 'catalogo' && (
          <div className="space-y-4 pt-2 w-full">
            <div className="bg-white p-6 rounded-[35px] shadow-md border w-full">
              ...
              }} className="w-full text-white p-4 rounded-2xl font-black uppercase text-xs shadow-md transition-colors" style={{ backgroundColor: corLojaPerfil }} disabled={subindoImagem}>
        ...
        {activeTab === 'materiais' && (
          <div className="space-y-4 pt-2 w-full">
            <div className="bg-white p-8 rounded-[40px] shadow-md border w-full">
              <h2 className="font-bold mb-4 flex items-center gap-2" style={{ color: corLojaPerfil }}><Package size={20}/> Gerenciar Armário</h2>
        ...
        {/* TELA DE PERFIL DA LOJA COM COLOR PICKER */}
        {activeTab === 'perfil' && (
          <div className="space-y-6 pt-2 w-full">
            <div className="bg-white p-6 rounded-[35px] shadow-md border w-full">
              <h2 className="font-bold mb-2 flex items-center gap-2 uppercase text-xs tracking-widest" style={{ color: corLojaPerfil }}><Settings size={18}/> Perfil da Minha Loja</h2>
      ...
      {/* MENU INFERIOR FIXO ATUALIZADO COM CORES DINÂMICAS */}
