...
export default function App() {
  ...
  const precoFinal = useMemo(() => {
    ...
    const total = ((cMat + mObra + ex) * Number(qtdPed || 1)) * (1 + (Number(lucro || 0) / 100)) - Number(desconto || 0);
    return isNaN(total) ? "0.00" : total.toFixed(2);
  }, [matsNoPed, vHora, tGasto, custos, lucro, qtdPed, desconto]);

  const enviarZap = (p: any) => {
    const cli = clientes.find(c => c.id === p.clienteId);
    const msg = `*ORÇAMENTO - PrecificaJá*%0A---%0A*Cliente:* ${cli?.nome || 'Cliente'}%0A*Produto:* ${p.nomeProd}%0A*Qtd:* ${p.qtdPed || 1} un%0A*Valor:* R$ ${p.preco}%0A*Pagamento:* ${p.pagamento || 'PIX'}%0A---%0AObrigado!`;
      ...
      <main className="p-4 max-w-xl mx-auto">
        {activeTab === 'criar' && (
          <div className="bg-white p-6 rounded-[35px] shadow-xl border mt-2">
            ...
            <input className="w-full p-4 bg-slate-50 rounded-2xl mb-4 outline-none" placeholder="Produto" value={nomeProd} onChange={e => setNomeProd(e.target.value)} />
            ...
            <select className="w-full p-4 bg-slate-50 rounded-2xl outline-none mb-4" onChange={e => {
            ...
            <div className="grid grid-cols-2 gap-4 mb-10">
               ...
               <input className="p-4 bg-slate-50 rounded-2xl outline-none" placeholder="Desconto R$" type="number" value={desconto} onChange={e => setDesconto(e.target.value)} />
            </div>
            <div className="flex items-center justify-between border-t pt-6">
              <div className="text-orange-500 font-black text-4xl">R$ {precoFinal}</div>
              <div className="flex gap-2">
                <button onClick={async () => {
                  await addDoc(collection(db, "pedidos"), { nomeProd, preco: precoFinal, clienteId: clienteSel, pagamento, qtdPed, userId: user.uid, data: new Date().toLocaleDateString() });
                  alert("Salvo!"); setMatsNoPed([]); setNomeProd('');
                }} className="bg-orange-500 text-white p-4 rounded-3xl font-black">SALVAR</button>
                <button onClick={() => enviarZap({nomeProd, preco: precoFinal, clienteId: clienteSel, pagamento, qtdPed})} className="bg-emerald-500 text-white p-4 rounded-3xl"><MessageCircle/></button>
              </div>
            </div>
          </div>
        )}
             ...
             {pedidos.map(p => {
               ...
               return (
                 <div key={p.id} className="bg-white p-5 rounded-[30px] shadow-sm flex justify-between items-center border">
                   <div>
                     <p className="font-black text-xs uppercase text-purple-700">{cli?.nome || 'Sem Cliente'}</p>
                   ...
                   <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => enviarZap(p)} className="text-emerald-500"><MessageCircle size={22}/></button>
                      <button onClick={() => deleteDoc(doc(db, "pedidos", p.id))} className="text-red-200"><Trash2 size={18}/></button>
                   </div>
                 </div>
               );
             })}
          ...
          <div className="space-y-4 pt-2">
            ...
            {materiais.map(m => (
              <div key={m.id} className="bg-white p-5 rounded-3xl flex justify-between items-center border">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setNovoMat({id: m.id, nome: m.nome, valor: m.valor, qtd: m.qtd})} className="text-orange-400"><Edit2 size={18}/></button>
        ...
        {activeTab === 'clientes' && (
          <div className="space-y-4 pt-2">
            <div className="bg-white p-8 rounded-[40px] shadow-md border">
              ...
              <input placeholder="Zap" className="w-full p-4 bg-slate-50 rounded-2xl mb-6 outline-none" value={novoCli.zap} onChange={e => setNovoCli({...novoCli, zap: e.target.value})} />
              <button onClick={async () => {
            ...
            {clientes.map(c => (
              <div key={c.id} className="bg-white p-5 rounded-3xl flex justify-between items-center border">
